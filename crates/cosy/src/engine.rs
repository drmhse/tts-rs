...
impl CosyVoiceEngine {
    pub fn load(config: &EngineConfig) -> Result<Self> {
        ...
        // The fixed initial noise is an asset, not a weight — see `crate::flow`.
        let noise_path = s(config.path("noise", "../../fixtures-cosy/rand_noise.safetensors"))?;
