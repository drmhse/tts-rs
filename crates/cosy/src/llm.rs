...
///
/// Prompts differ in length, so they are **right-aligned** — padded on the left so every
/// lane's last real token sits in the same column. That is what makes one shared position
/// index correct for all lanes at once: a RoPE score depends only on `p - j`, so shifting a
/// whole lane by a constant leaves every score it computes unchanged.
///
/// The argument rests on `R(p)^T R(j) = R(p - j)` holding in the table's arithmetic, and it
/// holds **to f32, not exactly**. `cosy-validate` measures it: an unpadded lane is
...
/// Audio8 batches the same way on bf16-rounded tables, where the identity only survives to
/// ~4e-3, so f32 buys about three orders of magnitude.
///
/// It is nevertheless exact where it counts: **greedy decoding of the whole utterance is
/// byte-identical between `llm_batch=1` and `llm_batch=7`**, because a 2e-6 perturbation
/// does not move an argmax. Sampled output *does* differ between the two, but only because
/// lanes draw from the shared RNG interleaved rather than one segment at a time — a
/// different draw, not a different model. Checking that distinction needs greedy; with
/// sampling alone the two are indistinguishable from a real regression.
...
impl Llm {
    ...
    /// column and one position index serves every lane. See [`Lane`] for why that is exact
    /// here and only approximate in Audio8.
    ...
    fn run(
        ...
        for (li, layer) in self.layers.iter().enumerate() {
            ...
            // in the file most likely to be wrong in a port that reuses `a8::ar`.
