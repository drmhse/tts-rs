//! The CosyVoice fixture gate: every stage against `fixtures-cosy/oracle.safetensors`.
//!
...
//!
//! Run: `cargo run -p cosy --release --bin cosy-validate`
...
fn main() -> Result<()> {
    ...
    let fx: HashMap<String, Tensor> = candle_core::safetensors::load(FIXTURES, &device)
        .with_context(|| format!("loading {FIXTURES} — run oracle-cosy/dump_fixtures.py"))?;
