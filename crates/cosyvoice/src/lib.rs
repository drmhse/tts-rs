...
//! precomputed into a voice asset by `references/cosyvoice/export_voice.py`, which is what keeps
//! 997 MB of ONNX (a whisper-based speech tokenizer and a speaker-embedding model) out of
//! this crate entirely.
//!
//! # Validation
//!
//! `cargo run -p cosyvoice --release --bin cosyvoice-validate` checks all three stages against
//! `fixtures/cosyvoice/oracle.safetensors`. The two rows that matter most:
//!
