...
//!
//! Run:  cargo run -p a8 --release --bin a8-validate -- [--cpu]

use a8::ar::{GenConfig, Model};
use a8::cfg;
use a8::codec::Codec;
use a8::nn::{max_abs_diff, Weights};
...
fn main() -> Result<()> {
    ...
    let fx = Weights::load("fixtures/oracle.safetensors", &Device::Cpu)?;
    println!("loaded {} fixture tensors", fx.len());

    let codec = Codec::load("oracle/weights/codec.safetensors", &device)?;
    ...
    println!("\n--- AR ---");
    let builder = PromptBuilder::load("oracle/weights/tokenizer.json")?;
    ...
    let model = Model::load("oracle/weights/model.safetensors", &device, None)?;
    ...
    // nothing about the part most likely to be wrong.
    {
        ...
        // batching.
        {
            let same: Vec<&a8::prompt::Prompt> = (0..4).map(|_| &prompts[0]).collect();
            let mut rng = Rng::new(1);
        ...
        {
            let f32rope =
                Model::load("oracle/weights/model.safetensors", &device, None)?.with_f32_rope()?;
            let mut solo32: Vec<Vec<Vec<u32>>> = Vec::new();
            for p in &prompts {
                let mut rng = Rng::new(1);
                solo32.push(f32rope.generate(p, &greedy, &mut rng)?);
            }
            let refs: Vec<&a8::prompt::Prompt> = prompts.iter().collect();
            let mut rng = Rng::new(1);
        ...
        for n in 0..40usize {
            for max in [1usize, 2, 4, 8] {
                let groups = a8::ar::plan_batches(n, max);
