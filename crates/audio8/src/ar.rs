...
impl Model {
    ...
    /// is what `audio8-validate` asserts. Any difference that survives under bf16 is thereby
    ...
    ///   output is unaffected, which is what `audio8-validate` checks: a batch of four distinct
...
/// quantized matmul.
///
/// Measured on the real loop (`tts-probe --bin arbatch`), that rule is wrong. Per-sequence
