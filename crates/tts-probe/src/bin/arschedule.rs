...
use audio8::ar::{plan_batches, GenConfig, Model};
...
const WEIGHTS: &str = "references/audio8/weights/model.safetensors";
const TOKENIZER: &str = "references/audio8/weights/tokenizer.json";
...
fn main() -> Result<()> {
    ...
    let prompts: Vec<audio8::prompt::Prompt> = segments
        .iter()
    ...
    let run = |max_batch: usize| -> candle_core::Result<()> {
        ...
        for group in plan_batches(order.len(), max_batch) {
            let refs: Vec<&audio8::prompt::Prompt> =
