...
//!
//! Run: `cargo run -p tts-probe --release --bin llmbatch`
...
fn main() -> Result<()> {
    ...
    for quant in ["q8_0", "none"] {
        ...
        for b in [1usize, 2, 4, 7] {
            ...
            let mut sampler = cosyvoice::sample::Sampler::new(cosyvoice::cfg::llm::VOCAB);
            ...
            let mut full = || -> candle_core::Result<()> {
                ...
                for lane in 0..b {
                    let mut sc = flat[lane * cosyvoice::cfg::llm::VOCAB
                        ..(lane + 1) * cosyvoice::cfg::llm::VOCAB]
                        .to_vec();
                    cosyvoice::sample::ras_sampling(&mut sampler, &mut sc, &hist, &mut rng);
                }
                let s = llm
                    .step_batch(s, &feed)
                    .map_err(|e| candle_core::Error::Msg(e.to_string()))?;
