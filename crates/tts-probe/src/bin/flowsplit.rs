...
//! 17%. So the flow is the only thing worth optimising, and the existing `cosyvoice-bench`
...
//! its share by ~16 relative to the projections. Whatever `cosyvoice-bench` says the split is,
//! it is not the split that matters.
//!
//! Run: `cargo run -p tts-probe --release --bin flowsplit`
