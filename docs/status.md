...
| Move CosyVoice's vocoder to CPU | ~7% of the engine | the vocoder is 56 s of a 745 s chapter. `overlap` shows two *GPU* stages do not overlap at all (1.00x), but CPU and GPU are different hardware — which is why the Python service forces `hift.inference` to CPU. Untried here |
