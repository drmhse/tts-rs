...
### ▶ [Hear it before you build it — one chapter, three engines, two cloned voices](https://drmhse.github.io/tts-rs/)

Same text, same laptop, nothing running but one Rust process. The fastest of the three turns a
1612-word chapter into 11 minutes of speech in 2m 40s.
...
`qwen3tts` is the one to start with: the best quality of the three and more than twice as fast
as either of the others on anything book-length. The [demo](https://drmhse.github.io/tts-rs/)
is there so you can disagree before spending a download on it.

One command, nothing manual. `bootstrap.sh` downloads and converts the checkpoint, fetches the
fixtures for its gates, and builds. **Add the others whenever you want them** — all three is
~13 GB, one is ~4 GB:
